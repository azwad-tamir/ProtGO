...
with open(r"./data/clustered_train_top100.pkl", "rb") as input_file:
...
# Loading true labels
val_df = pd.read_csv("./clustered_test_df")
...
def compute_metrics(eval_preds):
    ...
    # logits_main.append(logits2)
    # labels_main.append(labels1)
    total = 0
    correct = 0
...
for fold in fold_range:
    ...
    trainer_filepath = "./model/clus_top100_BP_BCE.model"
    ...
    # config = AutoConfig.from_pretrained("./model/top100_BP_BCE.model")
    # model2 = MyBertForSequenceClassification.from_pretrained("./model/top100_BP_BCE.model", config=config)
    ...
    all_labels_eval = []
    all_logits_eval = []
    for i in range(len(val_seq3)):
    # for i in range(1000):
        if i%100 == 0:
            print("VAL INDEX :: ",i)
        dict_curr = {'text': [val_seq3[i]], 'labels': [val_labels[i]]}
        val_seq_list.append(val_seq3[i])
        dataset_curr = Dataset.from_dict(dict_curr)
        tokenized_datasets_curr = dataset_curr.map(tokenize_function, batched=True)
        tokenized_datasets_curr.set_format("torch", columns=['input_ids'])
        tokenized_datasets_curr = tokenized_datasets_curr.remove_columns('text')
        tokenized_datasets_curr.set_format("torch")

        all_predictions_eval = trainer.predict(test_dataset=tokenized_datasets_curr)
        all_labels_eval.append(all_predictions_eval[1])
        all_logits_eval.append(all_predictions_eval[0])

    all_labels_array = all_labels_eval[0]
    all_logits_array = all_logits_eval[0]

    for i in range(len(all_labels_eval)-1):
        all_labels_array = np.concatenate((all_labels_array, all_labels_eval[i+1]), axis=0)
        all_logits_array = np.concatenate((all_logits_array, all_logits_eval[i+1]), axis=0)


    metrices_eval = compute_metrics([all_logits_array, all_labels_array])
    print("Test Accuracy: ", metrices_eval['accuracy'])

    with open('./results/r_clustered_top100_BP.pkl', "wb") as output_file:
        pickle.dump([all_logits_train, all_labels_train, all_logits_array, all_labels_array, val_seq_list], output_file)
        # pickle.dump([all_logits_array, all_labels_array, val_seq_list], output_file)


    #
    # with open(r"./results/r_top100_BP.pkl", "rb") as input_file:
    #     results = pickle.load(input_file)
    #
    # all_logits_train = results[0]
    # all_labels_train = results[1]
    # all_logits_array = results[2]
    # all_labels_array = results[3]
    # val_seq_list = results[4]
