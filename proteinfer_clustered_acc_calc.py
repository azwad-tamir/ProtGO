...
run_name = f'go_clustered_test_ens'
...
# final_pd.to_pickle("./data/proteinfer_ens_clustered_final_pd.pkl")
...
# final_pd = pd.read_pickle("./data/proteinfer_ens_clustered_final_pd.pkl")

# Reading random_vocab file:
with open('./data/go_clustered_vocab.pkl', 'rb') as fp:
...
# 0.005 --> 0.6115
'''
Min Decision threshold value::  0.005
Proteinfer Clustered test Accuracy:  0.611531366291706
pred_count:  3952869
True_count:  2735186
temp2:  197255
temp3:  1299773
temp4:  8412
'''
# min_decision_threshold_list = [7e-3, 1e-2, 2e-2, 5e-2, 7e-2, 1e-1, 3e-1, 5e-1]
...
proteinfer_random_pred_df = pd.read_csv("./data/proteinfer_clustered_test_go_pred.pkl")
...
# Loading true labels
true_df = pd.read_csv("./clustered_test_df")
t_id = list(true_df['ID'])
t_go = list(true_df['GO_terms'])

# Loading top100 aspectwise go terms:
with open('./data/clustered_train_top100.pkl', 'rb') as fp:
...
########################################################################################################################
## Calculating BP accuracy metrics
...
# print("Proteinfer Clustered test Accuracy: ", acc)
fn = temp1 + temp2
fp = temp3 + temp4
tp = correct
...
acc = correct/total
# print("Min Decision threshold value:: ", min_decision_threshold)
print("Proteinfer BP Clustered test Accuracy: ", acc)
...
########################################################################################################################
## Calculating MF accuracy metrics
...
for i in range(len(go_terms)):
    ...
    #     print("index: ", i)
    for j in range(len(go_terms[i])):
        if go_terms[i][j] in mf_top100_go:
            temp_go.append(go_terms[i][j])
...
acc = correct/total
# print("Min Decision threshold value:: ", min_decision_threshold)
print("Proteinfer MF Clustered test Accuracy: ", acc)
print("precision: ", precision)
print("Recall: ", recall)
print("F1_score", f1_score)

########################################################################################################################
## Calculating CC accuracy metrics
...
acc = correct/total
# print("Min Decision threshold value:: ", min_decision_threshold)
print("Proteinfer CC Clustered test Accuracy: ", acc)
print("precision: ", precision)
print("Recall: ", recall)
print("F1_score", f1_score)
